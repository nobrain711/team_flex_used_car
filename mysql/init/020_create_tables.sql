...
( /* 자동차 제조사 */
    maker_id     BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT '주식별자(PK)',
...
( /* 차량 제원 정보 */
    car_spec_id     BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT '주식별자(PK)',
...
( /* 중고차 매매 정보 */
    used_car_id    BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT '주식별자(PK)',
    car_spec_id    BIGINT UNSIGNED NOT NULL COMMENT 'car_specs 참조 식별자(FK)',
    price          INT             NOT NULL COMMENT '등록된 가격과 리스인 경우에 실질가격(원): 인수비용+(월 리스료 * 남은 리스개월)-승계지원금',
    listing_url    VARCHAR(255)    NOT NULL COMMENT '중고차 매매 사이트 링크',
    is_lease       TINYINT         NOT NULL DEFAULT 0 COMMENT '0=일반매물, 1=리스/렌트/할부승계 포함',
    mileage_km     INT             NOT NULL COMMENT '주행 거리 (Km)',
    color_name     VARCHAR(100)    NOT NULL COMMENT '차량의 색상',
    car_age_months INT             NOT NULL COMMENT '차량 연식(개월): 서비스 기준월 - 최초등록월'
) ENGINE = InnoDB;

CREATE TABLE IF NOT EXISTS leases
(/* 자동차 리스 관련 정보 테이블 */
    lease_id              BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY COMMENT '주식별자(PK)',
    used_car_id           BIGINT UNSIGNED NOT NULL COMMENT 'used_cars 참조 식별자(FK, 1:1)',
